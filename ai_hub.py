...
class AIHub:
    ...
    def _get_claude_client(self):
        if self._claude_client is None:
            import anthropic
            self._claude_client = anthropic.Anthropic(api_key=self.claude_api_key, timeout=60.0)
    ...
    _LANG_INSTRUCTION = (
        ...
        "DOCUMENT TEMPLATES: "
        "When the user requests a formal document (report, letter, business plan, manual, book, proposal), "
        "use structured document markers for professional formatting. "
        "START with [DOC:type] where type is one of: report, letter, plan, manual, book. "
        "For reports/plans/manuals/books, use: "
        "[COVER] title: Document Title / subtitle: Optional Subtitle / author: Author Name / date: Date / org: Organization [/COVER] "
        "Then use [SECTION:section-id:Section Title] content here [/SECTION] for each chapter/section. "
        "Use unique section IDs like section-1, section-2, market-analysis, etc. "
        "For letters, use [DOC:letter] and write with date, recipient, body, closing, and signature. "
        "DOCUMENT UPDATES: When the user asks to modify/update/add/delete parts of a previously created document, "
        "use these markers: "
        "[UPDATE:section-id]new content[/UPDATE] to replace a section's content. "
        "[DELETE:section-id] to remove a section. "
    ...
    TIER_LIMITS = {
        ...
        "executive":  {"personas": 10, "folders": 50,   "files": 500,  "storage_mb": 1000, "file_size_mb": 50},
        "ceo":        {"personas": 50, "folders": 9999, "files": 9999, "storage_mb": 9999, "file_size_mb": 100},
    }
